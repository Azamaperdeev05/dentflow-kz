...
const links = [
  ...
  { href: "/doctor/schedule", label: "Кесте", icon: "/icons/windows11-outline/schedule.png" },
  { href: "/doctor/patients", label: "Пациенттер", icon: "/icons/windows11-outline/patients.png" },
  { href: "/doctor/finance", label: "Қаржы", icon: "/icons/windows11-outline/finance.png" },
  { href: "/doctor/messages", label: "Хабарламалар", icon: "/icons/windows11-outline/messages.png" },
];
    ...
    <div className="mt-6 border-t border-slate-200 pt-4">
      ...
      >
        <Image
          ...
          width={18}
          height={18}
          className={`${pathname === "/doctor/profile" ? "opacity-100" : "opacity-80"}`}
        />
        <span>Профил</span>
      </Link>
    </div>
  ...
  return (
    <>
      <button
        ...
        className="fixed bottom-6 left-6 z-40 rounded-full bg-gradient-to-r from-emerald-600 to-teal-600 px-4 py-2 text-sm font-semibold text-white shadow-lg lg:hidden"
      >
        Меню
      </button>
        ...
        <div className="fixed inset-0 z-50 bg-slate-900/50 lg:hidden" onClick={() => setIsOpen(false)}>
          <aside className="h-full w-72 bg-white p-4 shadow-2xl" onClick={(event) => event.stopPropagation()}>
            <div className="mb-4 flex items-center justify-between">
              <p className="text-sm font-bold uppercase tracking-wide text-slate-500">Дәрігер мәзірі</p>
              <button
                type="button"
